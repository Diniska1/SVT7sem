...
double source(double x, double y)
{
    ...
    return sin(a * x) * sin(a * y) * (16 * dx * dy);
}
    ...
    for(Mesh::iteratorFace iface = m.BeginFace(); iface != m.EndFace(); iface++){
        ...
        if(f.Boundary()){
            ...
            if(BCtype == BC_NEUM){
                ...
                double bc = f.Real(tagBCval);
                double frac = calc_tf(D, nf, dA);
            ...
            double fracA = calc_tf(DA, nf, dA);
            double fracB = calc_tf(DB, nf, dB);
            double tf = (fracA * fracB) / (fracA - fracB);
